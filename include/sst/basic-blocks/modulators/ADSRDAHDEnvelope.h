...
struct ADSRDAHDEnvelope
{
    ...
    ADSRDAHDEnvelope(SRProvider *s) : srProvider(s)
    {
    ...
    {
        coeff_offset = 2.f - std::log2(srProvider->samplerate * BLOCK_SIZE_INV);
    }
                                   ...
                                   const bool gateActive)
    {
        ...
        case s_attack:
        {
            phase += srProvider->envelope_rate_linear_nowrap(a * etScale + etMin);
            if (phase > 1)
            {
                phase = 0;
                stage = s_decay;
        ...
        case s_decay:
        {
            phase += srProvider->envelope_rate_linear_nowrap(d * etScale + etMin);
            if (phase > 1)
            {
                phase = 0;
                stage = s_sustain;
        ...
        case s_release:
        {
            phase += srProvider->envelope_rate_linear_nowrap(r * etScale + etMin);
            if (phase > 1)
            {
                phase = 0;
                stage = s_eoc;
                                   ...
                                   const int ashape, const int dshape, const int rshape)
    {
        ...
        case s_delay:
        {
            phase += srProvider->envelope_rate_linear_nowrap(dly * etScale + etMin);
            if (phase > 1)
            {
                stage = s_attack;
                phase -= 1;
        ...
        case s_sustain:
        {
            phase += srProvider->envelope_rate_linear_nowrap(h * etScale + etMin);
            if (phase > 1)
            {
                phase = 0;
                stage = s_release;
